...
describe('Workout Session API CRUD operations', () => {
  ...
  it('should get a workout session by ID', async () => {
    const response = await request(app).get(`/api/workout-sessions/${workoutSessionId}`);
  ...
  it('should delete a workout session by ID', async () => {
    const response = await request(app).delete(`/api/workout-sessions/${workoutSessionId}`);
  ...
  it('should return 404 when trying to update a deleted workout session', async () => {
    const response = await request(app).put(`/api/workout-sessions/${workoutSessionId}`);
    expect(response.status).toBe(404);
  });
